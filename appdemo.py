...
# Function to find the next available ID for the output file
def find_next_id(jira_id):
    os.makedirs("outputs", exist_ok=True)
    files = [f for f in os.listdir("outputs") if f.startswith(f"{jira_id}_output")]
    ...
    for file in files:
        try:
            file_id = int(file.split("output")[1].split(".")[0])
    ...
    <script>
        ...
        document.getElementById('testCaseForm').addEventListener('submit', function(event) {
            ...
            .then(data => {
                ...
                // Check if there's already a download section and remove it
                const existingDownload = document.querySelector('.download-section');
                if (existingDownload) {
                ...
                downloadSection.innerHTML = `
                    <p>File saved as: ${data.file_name} (${data.token_count} tokens)</p>
                    <a href="/download/${data.file_name}" class="btn btn-download" download>Download Test Cases</a>
                `;
...
async def generate_test_cases(request: TestCaseRequest = Body(...)):
    try:
        ...
        # Find the next file ID and save the output
        file_id = find_next_id(request.jira_id)
        file_name = f"{request.jira_id}_output{file_id}.md"
        ...
        with open(f"outputs/{file_name}", "w") as file:
            ...
            file.write(content + "\n")
            file.write(f"Tokens Outputed: {token_count}\n")
            file.write("\n")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("appdemo:app", host="0.0.0.0", port=8000, reload=True)